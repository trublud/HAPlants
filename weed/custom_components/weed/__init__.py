...
async def async_setup(hass: HomeAssistant, config: dict):
    ...
    #topic = 'config'
    #mqttentity_id = 'homeassistant/sensor/'
    ...
    @callback
    def set_attribs_service(call: ServiceCall) -> None:
        """Service to send a message."""
        hass.components.mqtt.publish(hass, call.data.get('topic'),  call.data.get('payload'),0,True)
        _LOGGER.debug(call.data)
    # Service to publish a message on MQTT.
    @callback
    def make_plant_service(call: ServiceCall) -> None:
        """Service to send a message."""
        hass.components.mqtt.publish(hass, call.data.get('topic'),  call.data.get('payload'),0,True)

    # Register our service with Home Assistant.
    hass.services.async_register(DOMAIN, 'make_plant', make_plant_service)
    # Register our service with Home Assistant.
    hass.services.async_register(DOMAIN, 'set_attribs', set_attribs_service)
...
class PlantDevice(Entity):
    ...
    def __init__(self, hass: HomeAssistant, config: ConfigEntry) -> None:
        ...
        self._birth = config.data[FLOW_PLANT_INFO].get(ATTR_PLANT_BIRTH)
        self._lastfed = config.data[FLOW_PLANT_INFO].get(ATTR_PLANT_LASTFED)
        self._lastwatered = config.data[FLOW_PLANT_INFO].get(ATTR_PLANT_LASTWATERED)
        self._attr_plant_lastfed = config.data[FLOW_PLANT_INFO].get(ATTR_PLANT_LASTFED)
        self._attr_plant_lastwatered = config.data[FLOW_PLANT_INFO].get(ATTR_PLANT_LASTWATERED)
        self._attr_plant_birth = config.data[FLOW_PLANT_INFO].get(ATTR_PLANT_BIRTH)
        
        self._strain = config.data[FLOW_PLANT_INFO].get(ATTR_PLANT_STRAIN)
        self._attr_plant_strain = config.data[FLOW_PLANT_INFO].get(ATTR_PLANT_STRAIN)
        
        self._planter = config.data[FLOW_PLANT_INFO].get(ATTR_PLANT_PLANTER)
        self._attr_plant_planter = config.data[FLOW_PLANT_INFO].get(ATTR_PLANT_PLANTER)
        ...
        self.strain = self._config.options.get(
            ATTR_PLANT_STRAIN, self._config.data[FLOW_PLANT_INFO].get(ATTR_PLANT_STRAIN)
        )        
        self.planter = self._config.options.get(
            ATTR_PLANT_PLANTER, self._config.data[FLOW_PLANT_INFO].get(ATTR_PLANT_PLANTER)
        )
        self.lastfed = self._config.options.get(
            ATTR_PLANT_LASTFED, self._config.data[FLOW_PLANT_INFO].get(ATTR_PLANT_LASTFED)
        )
        self.lastwatered = self._config.options.get(
            ATTR_PLANT_LASTWATERED, self._config.data[FLOW_PLANT_INFO].get(ATTR_PLANT_LASTWATERED)
        )
        self.birth = self._config.options.get(
            ATTR_PLANT_BIRTH, self._config.data[FLOW_PLANT_INFO].get(ATTR_PLANT_BIRTH)
        )
    ...
    @property
    def device_lastfed(self) -> str:
        """The device_lastfed for all the entities"""
        return self._lastfed
    @property
    def device_lastwatered(self) -> str:
        """The device_lastwatered for all the entities"""
        return self._lastwatered
    @property
    def device_birth(self) -> str:
        """The birthdate for all the entities"""
        return self._birth

    @property
    def device_planter(self) -> str:
        """The amount of gallons for all the entities"""
        return self._planter

    @property
    def device_strain(self) -> str:
        """The strain for all the entities"""
        return self._strain
    ...
    @property
    def water_trigger(self) -> bool:
        """Whether we will generate alarms based on water"""
        return self._config.options.get(FLOW_WATER_TRIGGER, True)
    @property
    ...
    def extra_state_attributes(self) -> dict:
        ...
        attributes = {
            ...
            f"{ATTR_PLANT_LASTFED}": self.lastfed,
            f"{ATTR_PLANT_LASTWATERED}": self.lastwatered,
            f"{ATTR_PLANT_BIRTH}": self.birth,
            f"{ATTR_PLANT_STRAIN}": self.strain,
            f"{ATTR_PLANT_PLANTER}": self.planter,
            ...
            f"{ATTR_WATER}_status": self.water_status,
    ...
    def websocket_info(self) -> dict:
        ...
        response = {
            ...
            ATTR_WATER: {
                ATTR_MAX: self.max_water.state,
                ATTR_MIN: self.min_water.state,
                ATTR_CURRENT: self.sensor_water.state or STATE_UNAVAILABLE,
                ATTR_ICON: self.sensor_water.icon,
                ATTR_UNIT_OF_MEASUREMENT: self.sensor_water.unit_of_measurement,
                ATTR_SENSOR: self.sensor_water.entity_id,
            },
            ATTR_HUMIDITY: {
    ...
    def add_planter(self, planter: Entity | None) -> None:
        """Set new planter"""
        self.planter = planter

    def add_lastfed(self, lastfed: Entity | None) -> None:
        """Set new fed"""
        self.lastfed = lastfed
        
    def add_lastwatered(self, lastwatered: Entity | None) -> None:
        """Set new lastwatered"""
        self.lastwatered = lastwatered
        
    def add_birth(self, birth: Entity | None) -> None:
        """Set new birth"""
        self.birth = birth
    ...
    def add_thresholds(
        ...
        max_water: Entity | None,
        min_water: Entity | None,
    ...
    def update(self) -> None:
        ...
        if (
            self.sensor_water is not None
            and self.sensor_water.state != STATE_UNKNOWN
            and self.sensor_water.state != STATE_UNAVAILABLE
            and self.sensor_water.state is not None
        ):
            if float(self.sensor_water.state) < float(self.min_water.state):
                self.water_status = STATE_LOW
                if self.water_trigger:
                    new_state = STATE_PROBLEM
            elif float(self.sensor_water.state) > float(self.max_water.state):
                self.water_status = STATE_HIGH
                if self.water_trigger:
                    new_state = STATE_PROBLEM
            else:
                self.water_status = STATE_OK
