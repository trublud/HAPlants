...
async def async_setup_entry(
    ...
    pmaxw = PlantMaxWater(hass, entry, plant)
    pminw = PlantMinWater(hass, entry, plant)
...
class PlantMaxWater(PlantMinMax):
    """Entity class for max water threshold"""
    ...
    def __init__(
        ...
        """Initialize the component."""
        self._attr_name = (
            f"{config.data[FLOW_PLANT_INFO][ATTR_NAME]} {ATTR_MAX} {READING_WATER}"
        )
        self._default_value = config.data[FLOW_PLANT_INFO][FLOW_PLANT_LIMITS].get(
            CONF_MAX_WATER, DEFAULT_MAX_WATER
        )
        self._attr_unique_id = f"{config.entry_id}-max-water"
        self._attr_native_unit_of_measurement = PERCENTAGE
    ...
    @property
    def device_class(self):
        return f"{SensorDeviceClass.WATER} threshold"


class PlantMinWater(PlantMinMax):
    """Entity class for min conductivity threshold"""
    ...
    def __init__(
        ...
        """Initialize the component."""
        self._attr_name = (
            f"{config.data[FLOW_PLANT_INFO][ATTR_NAME]} {ATTR_MIN} {READING_WATER}"
        )
        self._default_value = config.data[FLOW_PLANT_INFO][FLOW_PLANT_LIMITS].get(
            CONF_MIN_WATER, DEFAULT_MIN_WATER
        )
        self._attr_unique_id = f"{config.entry_id}-min-water"
        self._attr_native_unit_of_measurement = PERCENTAGE
        super().__init__(hass, config, plantdevice)
        self._attr_native_max_value = 100
        self._attr_native_min_value = 0
