...
async def async_setup_entry(
    ...
    if SETUP_DUMMY_SENSORS:
        sensor_entities = [
            ...
            PlantDummyWater(hass, entry, plant),
        ]
    ...
    pcurw = PlantCurrentWater(hass, entry, plant)
    plant_sensors = [
        pcurb,
        pcurc,
        pcurm,
    ...
    _LOGGER.debug("Setting %s switz sensor to %s", plant.unique_id, plant)
    #hass.components.mqtt.publish(hass, "/homeassistant/sensor/"plant.unique_id,  "{ 'name': '"plant.name"', 'object_id' : '"plant.unique_id"', 'state_topic' : 'homeassistant/sensor/"plant.unique_id"/state', 'json_attributes_topic' : 'homeassistant/sensor/"plant.unique_id"/state', 'unique_id' : '420perms_"plant.unique_id"' }",0,True)
...
class PlantCurrentWater(PlantCurrentStatus):
    """Entity class for the current water meter"""
    ...
    def __init__(
        ...
        """Initialize the sensor"""
        self._attr_name = (
            f"{config.data[FLOW_PLANT_INFO][ATTR_NAME]} {READING_WATER}"
        )
        self._attr_unique_id = f"{config.entry_id}-current-water"
        self._external_sensor = config.data[FLOW_PLANT_INFO].get(FLOW_SENSOR_WATER)
...
class PlantDummyWater(PlantDummyStatus):
    """Dummy sensor"""
    ...
    def __init__(
        ...
        """Init the dummy sensor"""
        self._attr_name = (
            f"Dummy {config.data[FLOW_PLANT_INFO][ATTR_NAME]} {READING_WATER}"
        )
        self._attr_unique_id = f"{config.entry_id}-dummy-water"
        self._attr_icon = ICON_WATER
        self._attr_native_unit_of_measurement = PERCENTAGE
        super().__init__(hass, config, plantdevice)
